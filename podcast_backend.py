...
def get_podcast_summary(podcast_transcript):
  import openai
  instructPrompt = """
  You are a skilled summarizer, so I need you to summarize the following text in a way that's easy to read, this is the podcast episode:
  """
...
def get_podcast_guest(podcast_transcript):
  ...
  completion = openai.ChatCompletion.create(
    ...
    functions=[
    {
        ...
        "description": "Get information on the podcast guest, if there's one and it's reputable, using only their full name",
        "parameters": {
            "type": "object",
            "properties": {
                "guest_name": {
...
def get_podcast_highlights(podcast_transcript):
  ...
  request = podcast_transcript[:10000]
  instructPrompt = """
  I will give you a podcast transcript. Extract the highlights of this episode and any insights or important questions done. Use bulletpoints to list them:
  """
  request = instructPrompt + podcast_transcript
  chatOutput = openai.ChatCompletion.create(model="gpt-3.5-turbo-16k",
                                            messages=[{"role": "system", "content": "You are a helpful assistant."},
